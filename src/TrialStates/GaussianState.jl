...
"""
    monopole_state(
        H::Hermitian;
        particle_number=size(H, 1) ÷ 2,
        gap_tolerance=1e-8,
    )

Construct the fixed-particle-number Gaussian ground state of the
number-conserving one-particle Hamiltonian `H`.

If `H` has `M` rows, the returned [`GaussianState`](@ref) contains `M`
fermionic modes. The requested `particle_number` lowest one-particle orbitals
are filled. For a partially filled spectrum, the Fermi level must be separated
by more than `gap_tolerance`; otherwise the many-body ground state is not
uniquely selected by the particle number and an `ArgumentError` is thrown.

Internally, the chemical potential is placed in the Fermi gap and `H` is
embedded in the number-conserving Bogoliubov-de Gennes Hamiltonian
...
The returned state has no variational mean-field parameters. In particular,
passing the full `2LxLy × 2LxLy` result of
[`hamiltonian_aux_triangular_torus`](@ref) constructs a Gaussian state on
`2LxLy` spin-orbitals. That state does not directly match an `Lx × Ly`
spin-1/2 PPEPS. For a spin-conserving auxiliary Hamiltonian, an `LxLy × LxLy`
spin block may instead be passed to construct a Gaussian ansatz on the binary
spin configurations; this is a different state from the Gutzwiller-projected
two-spin Slater determinant.
"""
function monopole_state(
    H::Hermitian;
    particle_number::Integer=size(H, 1) ÷ 2,
    gap_tolerance::Real=1e-8,
)
    number_of_modes = size(H, 1)
    number_of_modes > 0 || throw(ArgumentError(
        "H must contain at least one fermionic mode",
    ))
    ...
    one_particle_hamiltonian = Matrix(H)
    all(isfinite, one_particle_hamiltonian) || throw(ArgumentError(
        "H must contain only finite matrix elements",
    ))
    energies = eigvals(H)
    energy_scale = max(maximum(abs, energies), one(eltype(energies)))

    chemical_potential = if particle_number == 0
        first(energies) - energy_scale
    elseif particle_number == number_of_modes
        last(energies) + energy_scale
    else
        ...
        (energies[particle_number] + energies[particle_number + 1]) / 2
    end

    particle_block = one_particle_hamiltonian - chemical_potential * I
    zero_block = zeros(eltype(particle_block), number_of_modes, number_of_modes)
    H_BdG = Hermitian([
        particle_block zero_block
        zero_block -transpose(particle_block)
    ])

    H_BdG_func = function (η, requested_number_of_modes)
        isempty(η) || throw(ArgumentError(
            "a frozen Gaussian ground state does not accept mean-field parameters",
        ))
        requested_number_of_modes == number_of_modes || throw(DimensionMismatch(
            "the BdG Hamiltonian contains $number_of_modes modes, " *
